...
nodemon은 여러분이 파일을 수정할 때 마다 자동으로 서버를 재시작한다. <u>yarn global add nodemon</u>로 설치.

제일 최상단 디렉토리에 index.js를 생성하고 의미 없는 console.log를 작성하고 ex) console.log(hello)  package.json에 `start`명령어를 추가하겠습니다.
...
console의 텍스트를 변경하면 nodemon이 자동으로 재실행.
...
그리고 [graphql-yoga의 git repository](https://github.com/prisma-labs/graphql-yoga)의 사용법을 참고하여 작성. 그 전에 최신 문법을 사용하기 위해 babel을 설치.  
...
[graphql-yoga의 git repository](https://github.com/prisma-labs/graphql-yoga)의 사용법을 참고하여 새로운 GraphQLServer를 만드는 `sever`라는 변수를 만든다.
...
우리가 방금 한 것은 어떤 사용자가 Query에 이름을 보내면 String을 보낸다는 설명을 했을 뿐이다.
Node.js에게 우리가 만든 Query의 기능성을 프로그래밍 해야한다.  

그래서 Query를 해결(resolve)해주는 **resolvers.js**라는 파일 생성.
Database에게 Query는 해결할 수 없는 문제라고 볼 수 있다. 그래서 우린 이 Query를 해결(resolve)해야 합니다.
...
해결(resolve)하고 싶은 문제는 name이고, 사용자가 name Query를 보내면 hj를 반환하는 함수를 만들었다.  
